                ...
                <div class="row">
                    <div class="table-container">
                        ...
                        function getFileIcon($extension)
                        {
                            ...
                            switch ($extension) {
                                case 'pdf':
                                    return 'pdf'; // File icon for PDF
                                case 'doc':
                                case 'docx':
                                    return 'word'; // File icon for Word documents
                                default:
                                    return 'alt'; // Generic file icon
                            }
                        }
                            ...
                            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) { // The code loops through the users table’s data until all the data has been displayed.
                             ...
                             <td>';
                                if (!empty($row['attachment'])) {
                                    ...
                                    echo '<a href="uploads/' . htmlspecialchars($row['attachment'], ENT_QUOTES, 'UTF-8') . '" download>
                                            <i class="fas fa-file-' . getFileIcon($fileExtension) . ' fa-2x" title="Download"></i>
                                          </a>';
                                } else {
                                    echo '<i class="fas fa-times-circle" title="No attachment"></i>';
                                }
                                echo '</td>
                             <td>' . $row['comments'] . '</td>
